...
func make_ready():
	...
	_audio = get_node_or_null('/root/main_scene/camera/audio')
	if _audio == null or _audio.is_queued_for_deletion():
		_audio = get_node('/root/main_menu/camera/audio')
...
func _go_to_designated_scene():
	...
	_colorize_screen.set_lerp_to_pos(_colorize_screen.global_position, _main_scene.linear_curve, self, _colorize_screen, 'fade_in')
