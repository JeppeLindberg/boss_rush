...
func _ready():
	...
	var player_ship_parts = _main_scene.get_children_in_groups(_game_space, ['player_ship_part'], true);
	player_ship_parts.shuffle();
	
	for part in player_ship_parts:
		var target_pos = part.global_position + Vector2.UP * 4 * _global_vars.GRID_SIZE_Y
...
func trigger():
	...
	_move(Vector2.DOWN * _movements[_movement_tick]);
	_movement_tick += 1;
